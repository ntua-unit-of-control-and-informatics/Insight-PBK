...
params_df = DataFrame(parameter = collect(keys(params)),
...
sol_df = DataFrame(time = solution.t)

# Check lengths and extract concentrations properly
println("Time points length: ", length(solution.t))
println("First concentration length: ", length(first(values(sol_dict))))

# Add concentration data to DataFrame
for (key, values) in sol_dict
    sol_df[!, key] = values
end

# Save to CSV
CSV.write("julia_model/julia_PBK_results.csv", sol_df)
println("Results saved to julia_PBK_results.csv")
