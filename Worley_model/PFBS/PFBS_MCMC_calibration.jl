...
println("="^80)
println("WORLEY PBK MODEL PARAMETER CALIBRATION - PFBS")
println("="^80)
...
# Extract PFBS partition coefficients
...
observation_concentration = 100.0  # μg/L
...
function run_pbk_worley(RAFbaso, RAFapi, exposure_dose_ng_kg_day, exposure_time_years, measurement_times_years, BW=70.0)
    ...
    user_input = (
        ...
        ingestion_time = [0.002],  # Start at time 0
        admin_dose = 0.0,
        admin_time = 0.0,
        admin_type = "oral",
        exp_type = "continuous",
...
println("="^80)
println("To run sampling, uncomment the following lines:")
println("")
println("# Sample from the model")
println("# chains = sample(worley_calibration(obs_data, log_obs), NUTS(), MCMCThreads(), n_iterations, n_chains)")
println("")
println("# Save results")
println("# using Serialization")
println("# serialize(\"Worley_PFBS_calibration_chains.jls\", chains)")
println("")
println("# Generate diagnostics")
println("# describe(chains)")
println("# chains_plot = plot(chains)")
println("# savefig(chains_plot, \"Worley_PFBS_calibration_diagnostic_plots.png\")")
...
println("\nSaving MCMC results...")
using Serialization
serialize("Worley_model/PFBS/Worley_PFBS_calibration_chains.jls", chains)
...
savefig(chains_plot, "Worley_model/PFBS/Worley_PFBS_calibration_diagnostic_plots.png")

println("\n" * "="^80)
println("CALIBRATION COMPLETE")
println("="^80)
println("Results saved to:")
println("  - Worley_PFBS_calibration_chains.jls (MCMC chains)")
println("  - Worley_PFBS_calibration_diagnostic_plots.png (trace plots and density plots)")
println("="^80)
