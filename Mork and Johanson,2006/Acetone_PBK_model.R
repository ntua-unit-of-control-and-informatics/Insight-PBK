...
#Compartment volumes (L)
  ...
  return(list(
    ...
    # 'PR'= PR,'C_bro_SS' = C_bro_SS,
    # 'C_muc_SS' = C_muc_SS,'C_alv_SS' = C_alv_SS,
    # 'C_liv_SS' = C_liv_SS, 'model_type'=model_type
...
#======================================================================================== 
   ...
   #respiratory tract (washin–washout model)
    ...
    if (model_type == "washin–washout") {  
    dA_bro <- - 2*Q_alv*C_bro - Q_alv*C_bro + Q_alv*C_muc/P_water_air
...
#I deleted Qc*C_art leaving from arterial blood CMT
    ...
    dA_art <- tissue_out - Q_alv*C_art/P_blood_air - Q_fat*C_art - Q_rich*C_art - 
    ...
    list(c( 'dA_fat' = dA_fat, 'dA_rich' = dA_rich, 'dA_rest' = dA_rest, 'dA_work' = dA_work,
            ...
            'dA_art' = dA_art, 'dA_exh' = dA_exh, 'dA_met' = dA_met, 'dPR'=dPR),
  ...
  with(as.list(parameters), {
    ...
    if (ltimes != ldose){
      stop("The times of administration should be equal in number to the doses")
       ...
       if (admin.type == "inh" & model_type == "washin–washout"){
        events <- list(data = rbind(data.frame(var = c("A_bro"),  time = admin.time, 
                                               value = admin.dose, method = c("add")) ))
    }else if (admin.type == "inh" & model_type == "inert-tube"){
        events <- list(data = rbind(data.frame(var = c("A_art"),  time = admin.time, 
                                                 value = admin.dose, method = c("add")) ))
...
Acetone_arterial_blood_WW_model_Ernstgard <- openxlsx::read.xlsx("Acetone_arterial blood_fit_Washin-washout_Ernstgard.xlsx")
Acetone_arterial_blood_IT_model_2a <- openxlsx::read.xlsx("Acetone_arterial blood_fit_Inert_tube_model_2a.xlsx")
Acetone_arterial_blood_IT_model_2a <- openxlsx::read.xlsx("Acetone_arterial blood_fit_Inert_tube_model_2a.xlsx")
Acetone_arterial_blood_WW_model_2a <- openxlsx::read.xlsx("Acetone_arterial blood_fit_Washin-washout_model_2a.xlsx")
Acetone_arterial_blood_IT_model_2b <- openxlsx::read.xlsx("Acetone_arterial blood_fit_Inert_tube_model_2b.xlsx")
Acetone_arterial_blood_WW_model_2b <- openxlsx::read.xlsx("Acetone_arterial blood_fit_Washin-washout_model_2b.xlsx")


setwd("Mork and Johanson,2006")

dataset <- list("df1" = Acetone_arterial_blood_IT_model_1, "df2" = Acetone_arterial_blood_WW_model_1,
                "df3" = Acetone_arterial_blood_WW_model_Ernstgard, "df4" = Acetone_arterial_blood_IT_model_2a,
                "df5" = Acetone_arterial_blood_WW_model_2a, "df6" = Acetone_arterial_blood_IT_model_2b,
                "df7" = Acetone_arterial_blood_WW_model_2b )
...
times_min <- seq(0, duration, 1)  # every min

admin.time <- times_min
admin.type <- "inh"
sex <- "M"
...
total_admin.dose <- exposure* params$Q_alv* duration
dose_each <- total_admin.dose / 121  # mg per pulse, pulse every min
admin.dose <- rep(dose_each, 121)
...
#################################
#--------------------------------
# Ernstgard et al., 1999 - Washin-washout model
#--------------------------------
#################################
...
colnames(exp_data)[c(2,3)] <- c("time", "concentration")
column_names <- c("C_art")
preds_Acetone_arterial_blood_WW_model_Ernstgard <- list()
...
for (i in 1:length(unique(exp_data$Tissue))) {
  ...
  preds_Acetone_arterial_blood_WW_model_Ernstgard [[i]] <- solution[solution$time %in% exp_time, column_names[i]]} #mg/L

obs_Acetone_arterial_blood_WW_model_Ernstgard <- list(exp_data[exp_data$Tissue == "Arterial_Blood", "concentration"]) #mg/L

#################################
#--------------------------------
# Wigaeus et al., 1981 - inert-tube model - series 2a
#--------------------------------
#################################
...
t_rest_min <- 30
t_ex_min   <- 90
times_min  <- seq(0, t_rest_min + t_ex_min, 1)  
admin.time <- times_min / 60
...
params_rest <- create.params(list(BW=BW, height=height, sex="M",
                                  workload=0, model_type=model_type,
                                  Cendo=Cendo))
params_ex   <- create.params(list(BW=BW, height=height, sex="M",
                                  workload=50, model_type=model_type,
                                  Cendo=Cendo))

inits <- create.inits(params)

Qalv_rest <- params_rest$Q_alv   # L/h (workload 0)
Qalv_ex   <- params_ex$Q_alv     # L/h (workload 50)

dose_per_min_rest <- exposure * Qalv_rest  # mg
dose_per_min_ex   <- exposure * Qalv_ex   # mg

n_rest <- t_rest_min          # 30 entries (minutes 1..30)
n_ex   <- t_ex_min          # 90 entries (minutes 31..120)

admin.time <- (1:(t_rest_min + t_ex_min)) 
admin.dose <- c(rep(dose_per_min_rest, n_rest),
                rep(dose_per_min_ex,   n_ex))
...
colnames(exp_data)[c(2,3)] <- c("time", "concentration")
column_names <- c("C_art")
preds_Acetone_arterial_blood_IT_model_2a <- list()
...
for (i in 1:length(unique(exp_data$Tissue))) {
  ...
  preds_Acetone_arterial_blood_IT_model_2a [[i]] <- solution[solution$time %in% exp_time, column_names[i]]} #mg/L

obs_Acetone_arterial_blood_IT_model_2a <- list(exp_data[exp_data$Tissue == "Arterial_Blood", "concentration"]) #mg/L

#################################
#--------------------------------
# Wigaeus et al., 1981 - washin-washout model - series 2a
#--------------------------------
#################################
...
colnames(exp_data)[c(2,3)] <- c("time", "concentration")
column_names <- c("C_art")
preds_Acetone_arterial_blood_WW_model_2a <- list()
...
for (i in 1:length(unique(exp_data$Tissue))) {
  ...
  preds_Acetone_arterial_blood_WW_model_2a [[i]] <- solution[solution$time %in% exp_time, column_names[i]]} #mg/L

obs_Acetone_arterial_blood_WW_model_2a <- list(exp_data[exp_data$Tissue == "Arterial_Blood", "concentration"]) #mg/L

#################################
#--------------------------------
# Wigaeus et al., 1981 - inert-tube model - series 2b
#--------------------------------
#################################
...
model_type <- "inert-tube"
Cendo <- 1  # mg/L (fixed value)
exposure <- 0.737 #mg/L
seg_min  <- 30
work_seq <- c(0, 50, 100, 150)
n_seg    <- length(work_seq)
n_total  <- n_seg * seg_min 
...
get_Qalv <- function(w){
  p <- create.params(list(
    BW = BW, height = height, sex = "M",
    workload = w, model_type = model_type,
    Cendo = Cendo
  ))
  p$Q_alv  # L/h
}

Qalv_vec <- vapply(work_seq, get_Qalv, numeric(1))
                   dose_per_min <- exposure * Qalv_vec  # mg per minute in each segment
                   
                   # --- build admin.time (hours) and admin.dose (mg) ---
                   admin.time <- (1:n_total)   # 1..120 min, in hours
                   
admin.dose <- c(rep(dose_per_min[1], seg_min),  # 0–30 min, workload 0
                rep(dose_per_min[2], seg_min),  # 30–60 min, workload 50
                rep(dose_per_min[3], seg_min),  # 60–90 min, workload 100
                rep(dose_per_min[4], seg_min))   # 90–120 min, workload 150
                
total_admin_dose <- sum(admin.dose)
events <- create.events(params)
...
colnames(exp_data)[c(2,3)] <- c("time", "concentration")
column_names <- c("C_art")
preds_Acetone_arterial_blood_IT_model_2b <- list()
...
for (i in 1:length(unique(exp_data$Tissue))) {
  ...
  preds_Acetone_arterial_blood_IT_model_2b [[i]] <- solution[solution$time %in% exp_time, column_names[i]]} #mg/L

obs_Acetone_arterial_blood_IT_model_2b <- list(exp_data[exp_data$Tissue == "Arterial_Blood", "concentration"]) #mg/L
...
colnames(exp_data)[c(2,3)] <- c("time", "concentration")
column_names <- c("C_art")
preds_Acetone_arterial_blood_WW_model_2b <- list()
...
for (i in 1:length(unique(exp_data$Tissue))) {
  ...
  preds_Acetone_arterial_blood_WW_model_2b [[i]] <- solution[solution$time %in% exp_time, column_names[i]]} #mg/L

obs_Acetone_arterial_blood_WW_model_2b <- list(exp_data[exp_data$Tissue == "Arterial_Blood", "concentration"]) #mg/L
...
times_min <- seq(0, duration, 1)  #every 1 min
...
preds_Acetone_arterial_blood_WW_model_Ernstgard <-  solution[, c("time","C_art")]
...
admin.time <- (1:(t_rest_min + t_ex_min))  # hours: 1..120 min
admin.dose <- c(rep(dose_per_min_rest, n_rest),
                rep(dose_per_min_ex,   n_ex))
...
preds_Acetone_arterial_blood_IT_model_2a <-  solution[, c("time","C_art")]

#################################
#--------------------------------
# Wigaeus et al., 1981 - washin-wasout model - series 2a
#--------------------------------
#################################
...
preds_Acetone_arterial_blood_WW_model_2a <-  solution[, c("time","C_art")]
...
preds_Acetone_arterial_blood_IT_model_2b <-  solution[, c("time","C_art")]

#################################
#--------------------------------
# Wigaeus et al., 1981 - washin-washout model - series 2b
#--------------------------------
#################################
# Set up parameters for event-based exposure
BW <- 73 #kg
...
preds_Acetone_arterial_blood_WW_model_2b <-  solution[, c("time","C_art")]
...
experiment3 <- reshape(Acetone_arterial_blood_WW_model_Ernstgard[c("Tissue" ,"Time_(min)", 
                                                         "Concentration_(mg/L)")], 
                       idvar = "Time_(min)", timevar = "Tissue", direction = "wide")
colnames(experiment3) <- c("Time",unique(Acetone_arterial_blood_WW_model_Ernstgard$Tissue))

experiment4 <- reshape(Acetone_arterial_blood_IT_model_2a[c("Tissue" ,"Time_(min)", 
                                                                   "Concentration_(mg/L)")], 
                       idvar = "Time_(min)", timevar = "Tissue", direction = "wide")
colnames(experiment4) <- c("Time",unique(Acetone_arterial_blood_IT_model_2a$Tissue))

experiment5 <- reshape(Acetone_arterial_blood_WW_model_2a[c("Tissue" ,"Time_(min)", 
                                                            "Concentration_(mg/L)")], 
                       idvar = "Time_(min)", timevar = "Tissue", direction = "wide")
colnames(experiment5) <- c("Time",unique(Acetone_arterial_blood_WW_model_2a$Tissue))

experiment6 <- reshape(Acetone_arterial_blood_IT_model_2b[c("Tissue" ,"Time_(min)", 
                                                            "Concentration_(mg/L)")], 
                       idvar = "Time_(min)", timevar = "Tissue", direction = "wide")
colnames(experiment6) <- c("Time",unique(Acetone_arterial_blood_IT_model_2b$Tissue))

experiment7 <- reshape(Acetone_arterial_blood_WW_model_2b[c("Tissue" ,"Time_(min)", 
                                                            "Concentration_(mg/L)")], 
                       idvar = "Time_(min)", timevar = "Tissue", direction = "wide")
colnames(experiment7) <- c("Time",unique(Acetone_arterial_blood_WW_model_2b$Tissue))
...
colnames(preds_Acetone_arterial_blood_WW_model_Ernstgard) <- c( "Time", "Arterial_Blood")
colnames(preds_Acetone_arterial_blood_IT_model_2a) <- c( "Time", "Arterial_Blood")
colnames(preds_Acetone_arterial_blood_WW_model_2a) <- c( "Time", "Arterial_Blood")
colnames(preds_Acetone_arterial_blood_IT_model_2b) <- c( "Time", "Arterial_Blood")
colnames(preds_Acetone_arterial_blood_WW_model_2b) <- c( "Time", "Arterial_Blood")
...
simulations <- list(predictions1 = preds_Acetone_arterial_blood_IT_model_1,
                    ...
                    predictions3 = preds_Acetone_arterial_blood_WW_model_Ernstgard,
                    predictions4 = preds_Acetone_arterial_blood_IT_model_2a,
                    predictions5 = preds_Acetone_arterial_blood_WW_model_2a,
                    predictions6 = preds_Acetone_arterial_blood_IT_model_2b,
                    predictions7 = preds_Acetone_arterial_blood_WW_model_2b)
